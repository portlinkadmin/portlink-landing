    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'linear-gradient(135deg,#1a4a6b,#2d6a98)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, color: 'white' }}>RV</div>
        </div>
      </div>
          ...
          return (
            <div key={s.label} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                ...
                <Icon size={14} color={s.accent ? 'rgba(255,255,255,0.7)' : '#2d6a98'} />
              </div>
      ...
      <div style={{ flex: 1, margin: '12px 14px 0', overflow: 'hidden' }}>
        <div style={{ background: '#fff', border: '1px solid #e8eef4', borderRadius: 12, overflow: 'hidden', height: '100%' }}>
          <div style={{ padding: '10px 12px', borderBottom: '1px solid #e8eef4', fontSize: 14, fontWeight: 600, color: '#0b1220' }}>Vessel arrivals</div>
          {arrivals.map((v, i) => {
            ...
            return (
              <div key={v.name} style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: '#6b87a0' }}>{v.berth} · {v.eta}</div>
                </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center', padding: '10px 14px 14px', background: '#fff', borderTop: '1px solid #e8eef4', marginTop: 10 }}>
        {[
          ...
          <div key={label} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
            <Icon size={18} color={active ? '#2d6a98' : '#aabbcc'} />
            <span style={{ fontSize: 10, color: active ? '#2d6a98' : '#aabbcc', fontWeight: active ? 600 : 400 }}>{label}</span>
          </div>
        ))}
      </div>
    </div>
